...
# Page 1: Clean glass sections (like your screenshot)
# Page 2: Water-style UI + LIME explanation rendered with Matplotlib
#         (NO exp.as_html() -> avoids blank white box on Streamlit Cloud)
# ============================================================
...
import matplotlib.pyplot as plt  # for LIME matplotlib figure
...
# ============================================================
    ...
    if run_btn:
        if user_text.strip() == "":
            ...
            # ✅ LIME Visual (Matplotlib) - works on Streamlit Cloud (no JS)
            st.markdown('<div class="water">', unsafe_allow_html=True)
            st.subheader("🧠 LIME Visual Explanation (No Blank Issue)")

            fig = exp.as_pyplot_figure()     # create matplotlib figure
            fig.set_facecolor("none")        # transparent bg so it fits water theme
            st.pyplot(fig, use_container_width=True)

            st.markdown("</div>", unsafe_allow_html=True)

            # Optional: simple highlighted text (bold top words)
            st.markdown('<div class="water">', unsafe_allow_html=True)
            st.subheader("📝 Highlighted Words (simple view)")

            top_words = [w for w, _ in exp.as_list()]
